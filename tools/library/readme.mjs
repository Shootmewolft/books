#!/usr/bin/env node
/**
 * Generates README.md from the library itself.
 *
 * The README this replaced was inherited from an unrelated repository: it
 * advertised books that did not exist, linked to another owner's GitHub, and
 * pointed at a BOOKS_LIST.md that was never there. Generating it from
 * book.json means the catalogue cannot drift from reality again.
 *
 *   node tools/library/readme.mjs [--check]
 *
 * --check exits non-zero if the committed README is stale, for CI.
 */
...
/** GitHub renders links relative to the repo root; paths need encoding. */
...
async function build() {
  ...
  lines.push('structure, rejects duplicates by hash, and enforces the naming rules, so a malformed')
